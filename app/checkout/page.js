  ...
  async function handleCheckout() {
    ...
    try {
      const res = await fetch('/api/payment/init', {
        ...
        body: JSON.stringify({ cartItems, totalPrice, deliveryCharge, customerInfo: form }),
      })
          ...
          <div style={{ marginBottom: '20px', padding: '16px', background: 'var(--bg)', borderRadius: '6px', border: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {['bKash', 'Nagad', 'Rocket', 'VISA', 'Mastercard'].map(method => (
                <span key={method} style={{ padding: '4px 10px', border: '1px solid var(--border)', borderRadius: '4px', fontSize: '11px', fontWeight: 600, color: 'var(--muted)' }}>{method}</span>
              ))}
            </div>
          </div>
