    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '60px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr min(360px, 100%)', gap: '40px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {cartItems.map(item => (
            <div key={`${item.id}-${item.size}`} style={{
              ...
              <div style={{
                width: '80px', height: '80px', background: '#1a1a2e',
              ...
              <div>
                <h3 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '20px', margin: '0 0 4px' }}>{item.name}</h3>
                <p style={{ color: 'var(--muted)', fontSize: '13px', margin: '0 0 12px' }}>Size: {item.size}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <button
                    onClick={() => updateQuantity(item.id, item.size, item.quantity - 1)}
                    style={{ width: '32px', height: '32px', background: 'var(--border)', border: 'none', borderRadius: '4px', color: 'var(--text)', fontSize: '18px', cursor: 'pointer', fontWeight: 700 }}>−</button>
                  <span style={{ fontWeight: 700, fontSize: '16px', minWidth: '20px', textAlign: 'center' }}>{item.quantity}</span>
                  <button
                    onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)}
                    style={{ width: '32px', height: '32px', background: 'var(--border)', border: 'none', borderRadius: '4px', color: 'var(--text)', fontSize: '18px', cursor: 'pointer', fontWeight: 700 }}>+</button>
                </div>
              ...
              <div style={{ textAlign: 'right' }}>
                <p style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '24px', color: 'var(--cyan)', margin: '0 0 8px' }}>৳{item.price * item.quantity}</p>
                <button
                  onClick={() => removeFromCart(item.id, item.size)}
                  style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: '12px', cursor: 'pointer', letterSpacing: '0.05em' }}>REMOVE</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '28px', position: 'sticky', top: '80px' }}>
          <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '28px', marginBottom: '24px' }}>ORDER SUMMARY</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--muted)', fontSize: '14px' }}>
              <span>Delivery</span>
              <span style={{ color: 'var(--cyan)' }}>Calculated at checkout</span>
            </div>
          </div>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: '16px', display: 'flex', justifyContent: 'space-between', marginBottom: '24px' }}>
            ...
            <span style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '28px', color: 'var(--cyan)' }}>৳{totalPrice}</span>
          </div>
          <button
          ...
          <Link href="/products" style={{ display: 'block', textAlign: 'center', color: 'var(--muted)', fontSize: '13px', textDecoration: 'none' }}>
            ← Continue Shopping
          </Link>
        </div>
      </div>
